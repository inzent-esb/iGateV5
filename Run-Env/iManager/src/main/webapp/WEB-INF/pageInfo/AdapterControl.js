	...
	search: {
		list: [
			...
			},			
			{
				...
				optionInfo: {
					'optionFor': "option in [ {text: 'File', value: 'F'}, {text: 'DB', value: 'D'}, {text: 'Memory', value: 'M'}, {text: 'Shared', value: 'S'} ]",
			...
			},
			{
				type: 'text',
				vModel: 'adapterDesc',
				label: this.$t('igate.adapter') + ' ' + this.$t('head.description'),
	...
	grid: {
		...
		options: {
    	    ...
    	    columns : [
    	      	...
    	      	{
    	      		name : 'status',
    	        	header : this.$t('head.queue') + ' ' + this.$t('head.status'),
    	        	align : 'center',
                    width: '10%',
    	        	formatter : function(name) {	        		
    	        		...
    	        		return '<div style="width:100%; height:100%; background-color:' + backgroundColor + ';color:' +fontColor +';">' 
    	    	...
    	    	{
                    name : 'adapterDesc',
    	        	header : this.$t('igate.adapter') + ' ' + this.$t('head.description'),
    	        	align : 'left',
                    width: '10%'
    	      	},   	      	
    	      	{
                    ...
                    formatter: function(info) {
                		return info.row.consumerCount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');;
                  	}
    	      	}, 
    	      	{
    	      		name : 'consumerMax',
                    ...
                    width: '10%',
                    formatter: function(info) {
                		return info.row.consumerMax.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');;
                  	}
    	      	}, 
    	      	{
    	      		name : 'messageCount',
                    ...
                    width: '10%',
                    formatter: function(info) {
                		return info.row.messageCount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');;
                  	}
    	      	}, 
    	      	{
    	      		name : 'messageMax',
                    ...
                    width: '10%',
    	      		formatter : function(name) {
    	      			...
    	      			else									 return name.row.messageMax.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');;
    	      		}
    	      	}, 
    	      	{
    	        	name : 'processResult',
