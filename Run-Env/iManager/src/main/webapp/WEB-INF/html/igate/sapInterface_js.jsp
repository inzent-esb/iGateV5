...
<script type="text/javascript">
  ...
  $(document).ready(function() {
	  ...
	  createPageObj.setViewName('sapInterface');
					...
					'detailSubList': [
						{
							...
							'changeEvt': "whenInterfaceIdChanged()",
							isPk: true
						}, 
						{
							'type': "select",
						...
						}, 
						{
							...
							'name': "<fmt:message>igate.interface</fmt:message> <fmt:message>head.description</fmt:message>"
						}, 
						{
							'type': "select",
							'mappingDataInfo': {
							...
							'name': "<fmt:message>igate.interface.metaDomain</fmt:message>"
						}, 
						{
							'type': "select",
							'mappingDataInfo': {
							...
							'name': "<fmt:message>igate.interface</fmt:message> <fmt:message>head.schedule</fmt:message> <fmt:message>head.type</fmt:message>"
						}
					]
				}, 
				{
					...
					'detailSubList': [
						{
							...
							'name': "<fmt:message>igate.interface.record.request</fmt:message>",
							'readonly': true
						}, 
						{
							'type': "text",
							'mappingDataInfo': "object.responseRecordId",
							'name': "<fmt:message>igate.interface.record.response</fmt:message>",
							'readonly': true
						}, 
						{
							'type': 'combination',
							'name': "<fmt:message>igate.interface.group</fmt:message>",
							'combiList': [
								{
									'type': "select",
									'mappingDataInfo': {
										'selectModel': 'object.interfaceGroup',
										'optionFor': 'option in interfaceGroups',
										'optionValue': 'option',
										'optionText': 'option'
									}
								}, 
								{
									'type': "text",
									'mappingDataInfo': "interfaceGroupEdit"
								}, 
							]
						}, 
						{
							...
							'name': "<fmt:message>igate.interface.execute.privilege</fmt:message>"
						}, 
						{
							'type': "search",
							'mappingDataInfo': {
						...
						}, 
						{
							...
							'name': "<fmt:message>igate.interface</fmt:message> <fmt:message>igate.adapter.disabledYn</fmt:message>" 
						}
					]
				}, 
	...
	window.vmSearch = new Vue({
		...
		methods: {
			...
			getStruct: function() {
				...
				$.ajax({
					type: "POST",
					url: "<c:url value='/igate/sapInterface/struct.json' />",
					processData: false,
					data: this.address,
	...
	window.vmMain = new Vue({
		...
		data: {
			...
			interfaceGroupsUri: "<c:url value='/igate/interface/groups.json'/>",
		...
		created: function() {
			...
			$.getJSON("<c:url value='/igate/interface/groups.json'/>", function(interfaceGroupData) {
				this.interfaceGroups = interfaceGroupData.object;
			}.bind(this));
		...
		methods: {
			...
			confirmRegist: function() {
				...
				var backupData = this.object.interfaceGroup;
				
				this.object.interfaceGroup = (this.object.interfaceGroup && this.interfaceGroupEdit)? this.object.interfaceGroup + "." + this.interfaceGroupEdit : this.interfaceGroupEdit;
				
				startSpinner();
				
				$.ajax({
					type: "POST",
					url: "<c:url value='/igate/sapInterface/regist.json'/>",
					processData: false,
					data: JsonImngObj.serialize($.extend({_method: 'Post'}, window.vmMain.object)),
					dataType: "json",
					success: function(result) {
						if ("ok" != result.result) {
							ResultImngObj.resultErrorHandler(result);
							this.object.interfaceGroup = backupData;
							stopSpinner();
							return;
						}
			...
			whenInterfaceIdChanged: function() {
				this.object.requestRecordId = "IFD_" + this.object.interfaceId + "_I";
				this.object.responseRecordId = "IFD_" + this.object.interfaceId + "_O";
			}
		}
	});
  });
