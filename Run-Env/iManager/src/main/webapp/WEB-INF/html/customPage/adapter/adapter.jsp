...
<body>
	<div id="adapter" data-ready>
		...
		<%@ include file="/WEB-INF/html/customPage/adapter/detail.jsp"%>
	</div>
	<script>
		document.querySelector('#adapter').addEventListener('ready', function(evt) {
			...
			SaveImngObj.setConfig({
			    objectUrl: '/api/entity/adapter/object'
			});

			ControlImngObj.setConfig({
			    controlUrl: '/api/entity/adapter/control',
			    dumpUrl: '/api/entity/adapter/dump'
			});

	        new HttpReq('/api/page/properties').read({ pk : { propertyId: 'List.Adapter.Type' }, orderByKey: true }, function (adapterListResult) {
	        	new HttpReq('/api/page/properties').read({ pk : { propertyId: 'List.Adapter.Queuemode' }, orderByKey: true }, function (queueModeListResult) {
					...
					window.vmMain = new Vue({
					    ...
					    methods: {
					        ...
					        changeAdapterType: function () {
					            ...
					            // charset
					            new HttpReq('/api/page/properties').read({ pk : { propertyId: 'Charset.Adapter.' + this.object.adapterType }, orderByKey: true },
					    ...
					    created: function () {
					        ...
					        new HttpReq('/api/page/properties').read(
					            { pk : { propertyId: 'Property.Adapter.Chartset' }, orderByKey: true },
					            function (result) {
					                this.adapterChartsetList = result.object;
					            }.bind(this)
					        );
					        new HttpReq('/api/page/properties').read(
					            { pk : { propertyId: 'List.Adapter.Endian' }, orderByKey: true },
					            function (result) {
					                this.adapterEndianList = result.object;
					            }.bind(this)
					        );
					        new HttpReq('/api/page/properties').read(
					            { pk : { propertyId: 'List.LogLevel' }, orderByKey: true },
					            function (result) {
					...
					window.vmAdapterOperations = new Vue({
					                    ...
					                    methods: {
					                        ...
					                        gridData: function (param) {
					                            ...
					                            for (var paramKey in param) {
					                                newDataObj[paramKey] = param[paramKey];
					                            }
					    ...
					    created: function () {
					        new HttpReq('/api/page/properties').read(
					            { pk : { propertyId: 'List.Adapter.Event' }, orderByKey: true },
					            function (result) {
					...
					window.vmAdapterProperties = new Vue({
					    ...
					    methods: {
					        ...
					        changeAdapterType: function () { // 기본정보 > 어댑터 타입 변경 이벤트
					        	this.setPropertyKeys(function() {
					        		this.adapterProperties = this.propertyKeys
					        ...
					        setPropertyKeys: function(callback) { //프로퍼티 키 datalist 불러오기
					        	new HttpReq('/api/page/properties').read(
			                        {
			                        	pk : { propertyId: 'Property.Adapter.' + vmMain.object.adapterType },
			                            orderByKey: true
			                        },
			...
			new Vue({
			    ...
			    methods: $.extend(true, {}, panelMethodOption, {
			        guide: function () {
			            window.open("${prefixUrl}/manual/Property Reference.htm", '_blank', 'height=886, width=785,resizable=yes, toolbar=no, menubar=no, location=no, scrollbars=yes, status=no');
			        },
