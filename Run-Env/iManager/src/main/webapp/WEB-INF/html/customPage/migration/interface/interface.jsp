				...
				<div class="form-group" style="margin-bottom: 16px;">
					<table class="migration-table" width="100%">
						...
						<tr v-for="interface in object.interfaces">
							...
							<td v-if="flagRow" :style="{color: interface.result ? 'red' : 'black' }" >{{interface.result ? interface.result : "<fmt:message>igate.migration.migration</fmt:message>"}}</td>
						</tr>
					</table>
				</div>
			</div>
			        ...
			        appendAreaList: [
			            {
			                getDetailArea: function () {
			                    ...
			                    detailHtml += '		       <th><fmt:message>igate.interface</fmt:message></th>';
			                    ...
			                    detailHtml += '           </table>';
			                    detailHtml += '       </td>';
			                    detailHtml += '      <td v-if="flagRow && isReulstUpdate" :style="{color: interface.result ? \'red\' : \'black\'}">';
		                    	detailHtml += '    	 	{{ interface.result ? interface.result : "<fmt:message>igate.migration.migration</fmt:message>" }}';
			                    detailHtml += '    	 </td>';
			                    detailHtml += '    </tr>';
			                    detailHtml += '</table>';
			                    detailHtml += '</div>';
			                    ...
			                    methods: $.extend(true, {}, listMethodOption, {
			                        ...
			                        make: function () {
			                            ...
			                            if (0 == targetIdList.length) {
			                            	...
			                            	MigrationImngObj.makeSubmit('/api/entity/interface/migration/object', targetIdList, false, '<fmt:message>igate.migration.make</fmt:message>', function (result) {
			    ...
			    methods: {
			        ...
			        migration: function () {
			        	openFlagModal(function(data) {
							...
							var url = '/api/entity/interface/migration/send' ; 
    						...
    						MigrationImngObj.makeSubmit(migrationUrl, targetIdList, data, '<fmt:message>igate.migration.migration</fmt:message>', function (result) {
    							...
    							window.vmMake.object.interfaces = result.object;
    			            });    						
			        	});
			        }
			    }
