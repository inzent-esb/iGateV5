			    ...
			    window.vmMain = new Vue({
			        ...
			        created: function () {
			            ...
			            if (localStorage.getItem('selectedOperation')) {
			                ...
			                SearchImngObj.load(this.selectedOperation);
			            }
			        },
			        methods: {
			    ...
			    window.vmWorkflow = new Vue({
			        ...
			        created: function () {
			            for (var key in this.imgObj) {
			                ...
			                this.imgObj[key].img.src = '${prefixUrl}/img/operation/' + this.imgObj[key].imgName + '.png';

			                if ('Activity' == key) {
			                    this.imgObj[key].imgCustom = new Image();
			                    this.imgObj[key].imgCustom.src = '${prefixUrl}/img/operation/activityCustom.png';
			                }
			            }
			        },
			        watch: {
			        ...
			        methods: {
			            refreshWorkflow: function () {
			                ...
			                if (0 < activityIdList.length) {
			                    $.ajax({
			                        type: 'GET',
			                        url: "${prefixUrl}/api/entity/activity/search",
			                        processData: false,
			                        data: (function () {
			                            var rtnParam = {};
			                    ...
			                    if (nodeInfo.selected()) {
			                        if ('Query' == nodeInfo._private.data.nodeType || 'Service' == nodeInfo._private.data.nodeType || 'Response' == nodeInfo._private.data.nodeType) {
			                            ...
			                            if (mappingId) {
			                                $.ajax({
			                                    type: 'GET',
			                                    url: "${prefixUrl}/api/entity/mapping/object",
			                                    processData: false,
			                                    data: $.param({
			                                    ...
			                                    success: function (result) {
			                                        if ('ok' == result.result) {
			                                            ...
			                                            localStorage.setItem('selectedMenuPathIdListNewTab', JSON.stringify(['100000', '101000', '101020']));
