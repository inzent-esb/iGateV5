	...
	<script>
		document.querySelector('#modelRecord').addEventListener('ready', function(evt) {
			...
			SaveImngObj.setConfig({
			    objectUrl: '/api/entity/record/object'
			});

			ControlImngObj.setConfig({
			    controlUrl: '/api/entity/record/control',
			    dumpUrl: '/api/entity/record/dump'
			});
		        ...
		        new HttpReq('/api/page/properties').read({ pk : { propertyId: 'List.Record.Type' }, orderByKey: true }, function (recordTypeResult) {
					...
					window.vmMain = new Vue({
					    ...
					    created: function () {
					        ...
					        new HttpReq('/api/page/properties').read({ pk : { propertyId: 'Property.Record' }, orderByKey: true }, function (result) {
					        ...
					        if (localStorage.getItem('selectedMessageModel')) {
					            ...
					            SearchImngObj.load(this.selectedMessageModel);
					        }
					    },
					    methods: {
					        loaded: function () {
			    ...
			    methods: {
			        ...
			        initRecordGridData: function (recordTreeGridData) {
			            if (!this.recordTreeGrid) {
			            	new HttpReq('/api/page/iToolsConfig').read(null, function(res) {
			            		var xmlDoc = (new DOMParser()).parseFromString(res.object, "text/xml");
		                        var config = JSON.parse(xml2json(xmlDoc, ' '));
